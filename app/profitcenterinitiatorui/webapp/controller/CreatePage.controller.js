      ...
      "mdg.profitcenter.ui.profitcenterinitiatorui.controller.CreatePage",
      {
        ...
        onFileChange: function (oEvent) {
          ...
          reader.onload = (e) => {
            ...
            this._processExcelData(excelRows);
          };
        ...
        _processExcelData: function (aRows) {
          ...
          aRows.forEach((row, index) => {
            ...
            if (!row["Profit Center "]) {
              errors.push("Profit Center is mandatory");
            }

            if (!/^[A-Za-z0-9]{1,20}$/.test(row.Name || "")) {
              errors.push("Name must be alphanumeric ≤ 20 chars");
            }

            if (!/^[A-Za-z0-9]{1,40}$/.test(row["Long Text"] || "")) {
              errors.push("Long Text must be alphanumeric ≤ 40 chars");
            }

            if (!row["Controlling Area"]) {
              errors.push("Controlling Area is mandatory");
            }

            if (!row.Segment) {
              errors.push("Segment is mandatory");
            }

            if (!this._isValidDate(row["Analysis Period From"])) {
              errors.push("Analysis Valid From is invalid");
            }

            if (!this._isValidDate(row["Analyis Period To"])) {
              errors.push("Analysis Valid To is invalid");
            }
          ...
          this._setExcelDataToModel(aValidData);
        },
    ...
    return day >= 1 && day <= maxDays;
        ...
        _mapExcelRow: function (row) {
          return {
            ...
            name: row.Name,
            analysisPeriodValidFrom: row["Analysis Period From"],
            analysisPeriodValidTo: row["Analyis Period To"],
            longText: row["Long Text"] || "",
